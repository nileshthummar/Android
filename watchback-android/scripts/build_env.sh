...
if grep -qi "dev*" <<< "${MATCHED_VAL}"; then
    echo "Building DEV"
    ./gradlew --stacktrace --info clean cleanBuildCache assembleDevRelease
     #lintDevRelease testDevReleaseUnitTest
else
    echo "Building PROD"
    ./gradlew --stacktrace --info clean cleanBuildCache assembleProdRelease
     #lintProdRelease testProdReleaseUnitTest
fi
